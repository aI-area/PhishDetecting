...
parser = argparse.ArgumentParser(
    description="Train the original URLNet architecture on an explicit shared split"
)
...
parser.add_argument('--data.train_csv', type=str, required=True, metavar="TRAINCSV")
parser.add_argument('--data.validation_csv', type=str, required=True, metavar="VALCSV")
...
parser.add_argument('--train.seed', type=int, default=42, metavar="SEED")
parser.add_argument(
    '--audit.counts_only',
    action='store_true',
    help='validate explicit inputs and print their counts without building the model',
)
...
def _csv_to_urlnet_txt(input_csv, url_col_name="url", label_col_name="phishing"):
    ...
    # Bind the intermediate name to the full source path and process identifier
    # so concurrent dataset runs cannot overwrite one another.
    source_id = hashlib.sha256(str(input_csv).encode("utf-8")).hexdigest()[:12]
    out_path = Path(tempfile.gettempdir()) / "{}_{}_{}_urlnet.txt".format(
        input_csv.stem, source_id, os.getpid()
    )
...
train_txt = _csv_to_urlnet_txt(FLAGS["data.train_csv"], "url", "phishing")
validation_txt = _csv_to_urlnet_txt(
    FLAGS["data.validation_csv"], "url", "phishing"
)
urls, labels = read_data(train_txt)
validation_urls, validation_labels = read_data(validation_txt)
for temporary_txt in (train_txt, validation_txt):
    try:
...
# The vocabulary is fitted above using training rows only.  Validation URLs are
# tokenized separately, then mapped only through the training dictionaries.
validation_x, validation_reverse_dict = get_word_vocab(
    validation_urls, FLAGS["data.max_len_words"]
)
validation_word_x = get_words(
    validation_x,
    validation_reverse_dict,
    FLAGS["data.delimit_mode"],
    validation_urls,
)
validation_ngramed_id_x, validation_worded_id_x = ngram_id_x_from_dict(
    validation_word_x,
    FLAGS["data.max_len_subwords"],
    ngrams_dict,
    words_dict,
)
validation_chared_id_x = char_id_x(
    validation_urls, chars_dict, FLAGS["data.max_len_chars"]
)

x_train_char = ngramed_id_x
x_test_char = validation_ngramed_id_x
x_train_word = worded_id_x
x_test_word = validation_worded_id_x
x_train_char_seq = chared_id_x
x_test_char_seq = validation_chared_id_x
y_train = to_categorical(labels, nb_classes=2)
y_test = to_categorical(validation_labels, nb_classes=2)

print("Explicit shared split: train={} validation={}".format(
    len(y_train), len(y_test)
))
print("Explicit label counts: train_benign={} train_phishing={} "
      "validation_benign={} validation_phishing={}".format(
          int(np.sum(np.asarray(labels) == 0)),
          int(np.sum(np.asarray(labels) == 1)),
          int(np.sum(np.asarray(validation_labels) == 0)),
          int(np.sum(np.asarray(validation_labels) == 1)),
      ))
if FLAGS["audit.counts_only"]:
    raise SystemExit(0)
...
with tf.Graph().as_default():
    tf.compat.v1.set_random_seed(FLAGS["train.seed"])
    ...
    with sess.as_default():
        ...
        train_batches, nb_batches_per_epoch, nb_batches = make_batches(
            ...
            FLAGS["train.batch_size"], FLAGS['train.nb_epochs'], True, True
        )
