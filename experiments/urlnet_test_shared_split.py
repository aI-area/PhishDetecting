...
parser.add_argument('--audit.expected_csv_sha256', type=str, default=None)
parser.add_argument('--audit.expected_rows', type=int, default=None)
parser.add_argument('--audit.counts_only', action='store_true')
parser.add_argument('--audit.allow_internal_duplicates', action='store_true')
    ...
    with input_csv.open("r", newline="", encoding="utf-8", errors="ignore") as f_in, \
        ...
        for row in reader:
            ...
            if url:
                ...
                f_out.write("{}\t{}\n".format(mapped, url))
                n_written += 1
                n_phishing += int(lab == 1)
    print("[convert] Wrote {} lines to {}".format(n_written, out_path))
    audit = {
        "csv_sha256": _sha256(input_csv),
        "rows": n_written,
        "phishing": n_phishing,
        "benign": n_written - n_phishing,
        "exact_duplicate_pairs": exact_duplicate_count,
    }
    return str(out_path), audit
...
if str(data_path).lower().endswith(".csv"):
    ...
    converted_path, cohort_audit = _csv_to_urlnet_txt(
        data_path, url_col_name=FLAGS["csv.url_col"], label_col_name=FLAGS["csv.label_col"]
    )
    FLAGS["data.data_dir"] = converted_path
    print("[audit] csv_sha256={csv_sha256} rows={rows} benign={benign} "
          "phishing={phishing} exact_duplicate_pairs={exact_duplicate_pairs}".format(
              **cohort_audit
          ))
    audit_error = None
    if FLAGS["audit.expected_csv_sha256"] is not None and (
        cohort_audit["csv_sha256"].lower() != FLAGS["audit.expected_csv_sha256"].lower()
    ):
        audit_error = "Target CSV SHA-256 does not match canonical manifest"
    elif FLAGS["audit.expected_rows"] is not None and (
        cohort_audit["rows"] != FLAGS["audit.expected_rows"]
    ):
        audit_error = "Target CSV row count does not match canonical manifest"
    elif (cohort_audit["exact_duplicate_pairs"] != 0 and
          not FLAGS["audit.allow_internal_duplicates"]):
        audit_error = "Target CSV contains exact duplicate URL/label pairs"
...
if FLAGS["audit.counts_only"]:
    print("[audit] URLNet parsed rows={} benign={} phishing={}".format(
        len(labels), int(np.sum(np.asarray(labels) == 0)),
...
with graph.as_default():
    ...
    with sess.as_default():
        ...
        # fetch placeholders / tensors by name (must match TextCNN)
...
def _binary_metrics(y_true, y_pred, y_score_pos):
    ...
    # ROC AUC (no sklearn): trapezoid over ROC curve
    pos = (y_true == 1).sum()
    neg = (y_true == 0).sum()
    if pos == 0 or neg == 0:
        roc_auc = float('nan')
...
# Convert raw scores -> positive-class probabilities using the same softmax as save_test_result()
probs_pos = []
for s in all_scores:
    # s is typically a length-2 vector of logits; utils.softmax returns np.array
    try:
        p1 = softmax(s)[1]
    except Exception:
...
# Ensure output dir, save detailed per-URL results (label/predict/score)
