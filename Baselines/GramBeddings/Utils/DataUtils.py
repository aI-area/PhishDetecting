...
def GetDataAndLabelsFromFiles(file_path, max_sequence_length=None, convert_to_array=True, label_phish=1, label_legit=0, split_char='\t'):
    """
    Reads data from a CSV file with 'url' and 'phishing' columns.
    Preserves original logic: 
    - Ignores URLs shorter than 8 chars.
    - Trims URLs to max_sequence_length.
    """
    samples = []
    labels = []
    
    # We ignore the split_char argument since we are now using standard CSV parsing
    with open(file_path, encoding="utf8", errors='ignore') as fp:
        ...
        for row in reader:
            try:
                # Access columns by name as requested
                sample = row['url']
                # Convert 'phishing' column to int (1 or 0)
                label = int(row['phishing'])
            except (KeyError, ValueError):
                # Skip rows that differ from expected format or have bad data
                continue

            # PRESERVE ORIGINAL LOGIC: Filter URLs shorter than 8 chars
            if len(sample) < 8:
                continue

            # PRESERVE ORIGINAL LOGIC: Trim to max sequence length
            if max_sequence_length is not None:
                sample = sample[:max_sequence_length]
...
class DatasetOptions(Enum):
    # IMPORTANT: Add your new dataset folders here
    # Format: NAME = 'Folder_Name_In_Data_Directory'
    DATASET_A = 'Dataset_A'
    DATASET_B = 'Dataset_B'
    
    # Original datasets preserved below
    grambeddings = 'grambeddings'
    grambeddings_adv = 'grambeddings_Adversarial'
    ebubekir = 'ebubekir'
    kd01 = 'kaggle/01-sid321axn'
