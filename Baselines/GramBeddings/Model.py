...
class NBeddingModel:
    ...
    def CreateModel(self, embedding_layer_name=None):
        ...
        embedding_layer = Embedding(
            self.vocab_size,
            self.embedding_dim,
            # FIXED: Removed deprecated 'input_length' argument
            weights= [self.embedding_matrix] if self.warm_start else None,
            mask_zero=True,
            name=embedding_layer_name,
        )
        ...
        # --- FIX START ---
        # Replaced GlobalMaxPooling1D with MaxPooling1D.
        # GlobalMaxPooling1D flattens the output to 2D (batch, features), losing the time dimension.
        # Conv1D and LSTM require 3D input (batch, time, features).
