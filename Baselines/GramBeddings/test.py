...
# Hardcoded paths
VOCAB_SOURCE_FILE = "/data/PhishFusion.csv"
TEST_TARGET_FILE = "/data/verified_Phishtank.csv"
...
def Process(args):
    ...
    print(f"Attempting to load model from: {file_name}")
    try:
        model = tf.keras.models.load_model(file_name, custom_objects={'ZhangAttention': ZhangAttention})
    except OSError:
        print(f"\nCRITICAL ERROR: Could not find model file at: {file_name}")
        print("Please ensure you ran train.py first and that it finished successfully.")
        return

    ####################################### Loading Dataset  #######################################
    print('####################################### Loading Dataset  #######################################')
    
    print(f"Loading vocabulary source from: {VOCAB_SOURCE_FILE}")
    fs_samples, fs_labels   = GetDataAndLabelsFromFiles(VOCAB_SOURCE_FILE)
    
    print(f"Loading test targets from: {TEST_TARGET_FILE}")
    test_samples, test_labels = GetDataAndLabelsFromFiles(TEST_TARGET_FILE)

    # Convert test samples to list/array
    ...
    print("Fitting input data in transformer to select best ngrams for n = ", args.ngram_1)
    # Fit on TRAINING samples to match model vocabulary
    selected_ngrams_1, selected_ngram_scores_1, weight_matrix_1, vocab_size_1, idf_dict_1 = transformer_1.Fit(
        fs_samples, fs_labels)
    print("Starting convert test texts to sequences for n = ", args.ngram_1)
    # Transform TEST samples
    ...
    selected_ngrams_2, selected_ngram_scores_2, weight_matrix_2, vocab_size_2, idf_dict_2 = transformer_2.Fit(
        fs_samples, fs_labels)
    print("Starting convert test texts to sequences for n = ", args.ngram_2)
    # Transform TEST samples
    ...
    selected_ngrams_3, selected_ngram_scores_3, weight_matrix_3, vocab_size_3, idf_dict_3 = transformer_3.Fit(
        fs_samples, fs_labels)
    print("Starting convert test texts to sequences for n = ", args.ngram_3)
    # Transform TEST samples
    ...
    results = model.evaluate([test_sequences_char, test_sequences_1, test_sequences_2, test_sequences_3], test_labels, batch_size=128, verbose=1)
