...
# Paths modified to be hardcoded as requested
TRAIN_SOURCE_FILE = "/data/PhishFusion.csv"

dataset_name = 'PhishFusion'
out_dir = 'outputs'
CHAR_EMBEDDING_DIM = 95
loss = "binary_crossentropy"
optimizer = "Adam"
...
def Process(args):
    ...
    # --- MODIFIED LOGIC: Load one file and split it 80/20 for Train/Val ---
    print(f"Loading data from: {TRAIN_SOURCE_FILE}")
    all_samples, all_labels = GetDataAndLabelsFromFiles(TRAIN_SOURCE_FILE)
    
    # Split the data (Shuffle is True by default in train_test_split)
    train_samples, val_samples, train_labels, val_labels = train_test_split(
        all_samples, all_labels, test_size=0.2, random_state=SEED
    )
    
    print(f"Data Splitting Completed: {len(train_samples)} Training samples, {len(val_samples)} Validation samples")
    # ---------------------------------------------------------------------
    ...
    # --- Added F1 Calculation for Output ---
    ...
    print(f"Accuracy:       {best_valid_accu:.4f}")
