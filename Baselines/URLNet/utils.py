...
def read_data(file_dir):
    # Use 'utf-8' with errors='ignore' to handle weird characters in URLs
    with open(file_dir, 'r', encoding='utf-8', errors='ignore') as file:
        urls = []
        labels = []
        for line_num, line in enumerate(file): # Use enumerate to track line numbers
            line = line.strip()
            if not line:
            ...
            # Safety Check: Ensure the line has at least 2 parts
            if len(items) < 2:
                print(f"Warning: Line {line_num} skipped (format error): {line[:50]}...")
                continue

            try:
                # Try to convert the label to an integer
                label = int(items[0])
            except ValueError:
                # If conversion fails (e.g., header row or garbage data), skip the line
                print(f"Warning: Line {line_num} skipped (not an integer label): {items[0]}")
                continue
            ...
            # Join the rest of the items just in case the URL itself contained tabs
            url = "".join(items[1:])
            urls.append(url)
