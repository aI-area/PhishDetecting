...
class StealthPhisherExtractor:
    def __init__(self):
        # Initialize TLD extract in offline mode
    ...
    def process_url(self, url):
        ...
        try:
            ...
            # --- URL NORMALIZATION (Fixes the specificty=0 issue) ---
            # If raw domain (e.g. "google.com"), assume HTTPS to match training data distribution
            if not url_str.startswith(('http://', 'https://')):
                target_url = 'https://' + url_str
                original_has_ssl = 1 
            else:
                target_url = url_str
                original_has_ssl = 1 if url_str.startswith('https://') else 0

            parsed = urlparse(target_url)
            ext = self.tld_extractor(target_url)
            domain = f"{ext.domain}.{ext.suffix}"
            path = parsed.path
            ...
            # Use string-based SSL detection
            features['HasSSL'] = original_has_ssl

            # Offline Mode: Zero out network features
            features['IsUnreachable'] = 0
            features['LineOfCode'] = 0
            features['LongestLineLength'] = 0
            features['HasTitle'] = 0
...
def extract_features_parallel(df, url_col='url', workers=50):
    total_urls = len(df)
    print(f"\n[INFO] Extracting features for {total_urls} URLs (Offline Mode)...")
